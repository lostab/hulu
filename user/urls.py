...
urlpatterns = [
    ...
    #url(r'^password_reset_confirm/(?P<uidb64>[0-9A-Za-z_\-]+)-(?P<token>.+)/$', authviews.password_reset_confirm, name='password_reset_confirm'),
    url(r'^password_reset_confirm/(?P<uidb64>[0-9A-Za-z_\-]+)/(?P<token>[0-9A-Za-z]{1,13}-[0-9A-Za-z]{1,20})/$', authviews.password_reset_confirm, name='password_reset_confirm'),
